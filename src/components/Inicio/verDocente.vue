<template>
    <div class="container">
        ...
        <div v-for="(docente, index) in docentesFiltrados" :key="index" class="docente-card">
          <div class="card-content">
            ...
            <div class="detalle-container">
              <h2>{{ docente.especialidad }}</h2> <!-- Especialidad primero -->
              <div class="detalle"><strong>Docente:</strong> <span class="detalle-valor">{{ docente.nombre }}</span></div>
              <div class="detalle"><strong>Salón:</strong> <span class="detalle-valor">{{ docente.salon }}</span></div>
              <div class="detalle"><strong>Turno:</strong> <span class="detalle-valor">{{ docente.turno }}</span></div>
            </div>
          </div>
          <div class="acciones">
      ...
      <div v-else>
        <p>No se encontraron docentes con esa especialidad.</p>
      </div>
    </div>
  </template>
  ...
  export default {
    ...
    methods: {
      ...
      editarDocente(index) {
        alert(`Editando docente: ${this.docentesFiltrados[index].nombre}`);
      },
      eliminarDocente(index) {
        if (confirm(`¿Seguro que deseas eliminar al docente ${this.docentesFiltrados[index].nombre}?`)) {
  ...
  .detalle-container {
    display: grid;
    grid-template-columns: 180px 1fr; /* Aumentado el espacio para las etiquetas */
